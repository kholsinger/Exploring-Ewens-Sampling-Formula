...
##
construct_a <- function(dna_seqs) {
  n_seqs <- nrow(dna_seqs)
  dna_seqs_chr <- character(n_seqs)
  for (i in 1:n_seqs) {
    dna_seqs_chr[i] <- paste(dna_seqs[1, ], collapse = "")
  }
  ##
  ...
  ##
  print()
  allele_table <- table(dna_seqs_chr)
  print(seg.sites(dna_seqs))
  print(allele_table)
  stop()
}
                     ...
                     shape = 1, rate = 1) 
{
  ...
  for (rep in 1:n_rep) {
    ...
    a <- matrix(nrow = n_loci, ncol = n_loci)
    for (l in 1:n_loci) {
