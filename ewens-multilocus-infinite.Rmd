...
We'll simulate $L$ independently inherited loci, using an infinite sites model for each. We'll estimate $\theta$ from the number of segregating sites, $\theta_K$, from the nucleotide diversity and from Ewen's sampling formula, $\theta_B$, where we assume the same $\theta_B$ across all loci.
...
p <- ggplot(for_plot, aes(x = value, fill = Model, color = Model)) +
  ...
  geom_vline(xintercept = 0.1, color = "salmon", lintype = "dashed") +
  theme_bw()
p
```
